...
class ShahrBank extends banking{
    ...
    public function __construct(array $data,$banking_id)
    {
    ...
    function isSignedIn()
    {
        ...
        if(strpos($homePage, $this->account) !== false) {
            ...
            $deposits = $this->logStatements();
            $GLOBALS['deposits'] = $deposits;
            return true;
        } else {
            return false;
        }
    ...
    {
        unset($_SESSION[$this->bankName .'_token_'.$this->banking_id]);
    ...
    private function getBalances()
    {
        ...
        return getBalanceFromDeposit($depositShow);
    }
    ...
    function getDataFromSigninPage(string $signinPage)
    {
        ...
        if($loginData['has_captcha']){
            writeOnFile($this->captchaFile, $loginData['captchaRawImage']);
        }
        return $loginData;
    }
